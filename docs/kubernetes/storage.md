...
With Rook Ceph, we provide block, filesystem, and object storage (S3-compatible). Block and filesystem storage are provisioned via CSI with persistent volumes, while object storage is provisioned via [COSI](https://github.com/kubernetes-sigs/container-object-storage-interface) with the object storage API.
